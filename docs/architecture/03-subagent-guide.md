...
- **Đồng thời**: Tối đa 1 sub-agent chạy đồng thời (mặc định, cấu hình qua `maxConcurrent`)
- **Có timeout**: Có thể đặt timeout để tự động dừng
- **System prompt riêng**: Sub-agent nhận prompt mode `"minimal"` (rút gọn so với main agent)
...
|---------|------|--------|----------|
| `maxConcurrent` | `number` | Số sub-agent chạy đồng thời tối đa (global lane: "subagent") | `1` |
...
| `thinking` | `string` | Mức thinking mặc định (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`) | `"off"` |
...
| `thinking` | `string` | Không | Override thinking level: `off`, `minimal`, `low`, `medium`, `high`, `xhigh` |
| `runTimeoutSeconds` | `number` | Không | Tự động dừng sau N giây (≥ 0) |
| `timeoutSeconds` | `number` | Không | Deprecated alias cho `runTimeoutSeconds` (back-compat) |
| `cleanup` | `"delete" \| "keep"` | Không | Chính sách dọn dẹp session. Mặc định: `"keep"` |

> **Lưu ý:** Các tham số `target`, `transport`, `channel`, `to`, `threadId` không được hỗ trợ và sẽ trả lỗi nếu sử dụng.
...
|----------|---------|---------|
| Max concurrent | 1 (mặc định) | Cấu hình qua `maxConcurrent` |
| Announce timeout | 30 giây | Thời gian đợi sub-agent hoàn thành trước khi announce |
| Gateway call timeout | 10 giây | Timeout cho mỗi RPC call (session.delete, sessions.patch) |
| Archive sweep | 60 giây | Tần suất quét sessions cũ |
| Default archive | 60 phút | Thời gian giữ session trước khi tự xóa |

### 8.4 System Prompt Mode

Sub-agents nhận system prompt ở chế độ `"minimal"` (thay vì `"full"` như main agent):

| PromptMode | Mô tả | Dùng cho |
|------------|--------|----------|
| `"full"` | Tất cả sections (identity, tools, skills, memory, messaging, reply tags) | Main agent |
| `"minimal"` | Rút gọn (Tooling, Workspace, Runtime) | Sub-agents |
| `"none"` | Chỉ identity line cơ bản | Đặc biệt |
